...
import shelve
shelvename = 'Chatting-shelve'
fieldnames = ('ID', 'Password')

def makeWidgets():
    global entries
    window = Tk()
    window.title('User Shelve')
    form = Frame(window)
    form.pack()
    entries = {}
    ...
    Button(window, text="Login", command=updateRecord).pack(side=LEFT)
...
def updateRecord():
    ...
    if key in db:
        ...
        from User import User             # make/store new one for key
        record = User(ID='?', Password='?')    # eval: strings must be quoted
    for field in fieldnames:
